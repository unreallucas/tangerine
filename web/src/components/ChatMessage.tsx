...
const markdownComponents: Components = {
  ...
  pre: ({ children }) => (
    <InsidePreContext.Provider value={true}>
      <pre className="my-2 rounded-md bg-muted p-3 font-mono text-sm leading-[1.6] overflow-x-auto border border-border not-italic">
        {children}
      </pre>
  ...
  code: ({ children, className }) => {
    ...
    if (!className && !insidePre) {
      return <code className="rounded bg-muted px-1 py-0.5 font-mono text-sm border border-border not-italic">{children}</code>
    }
